  ...
  <div class="mx-auto max-w-3xl p-4 animate-in" class:pb-24={selectedSet.size > 0}>
    ...
    <div class="flex items-center gap-2 mb-3 flex-wrap text-sm">
      <button class="inline-flex items-center gap-1.5 px-3 py-1 border border-border rounded-lg text-sm cursor-pointer hover:border-primary hover:text-primary transition-colors bg-transparent" on:click={toggleFiltered}>
        <span class="text-base leading-none">{allFilteredSelected ? '☑' : '☐'}</span>
      ...
      {#each filteredVocab as item, index}
        ...
        >
          ...
          <div class="flex flex-col items-end gap-2">
            ...
            {#if hasAudio}
              <button class="border border-border rounded-lg px-2 py-1 cursor-pointer text-base hover:border-primary transition-colors bg-transparent" on:click|stopPropagation={() => playJapaneseAudio(item.kana || item.japanese)} title="Listen">
                🔊
              </button>
            {/if}
          </div>
...
{#if selectedSet.size > 0}
  <div class="practice-bar fixed bottom-0 inset-x-0 bg-card border-t-2 border-primary px-4 py-2.5 flex items-center gap-3 z-50 shadow-lg">
    ...
    <div class="flex gap-2 flex-1 justify-center flex-wrap">
      <button class="px-3.5 py-2 bg-primary text-white border-none rounded-xl cursor-pointer text-sm font-semibold whitespace-nowrap hover:brightness-90 transition-all" on:click={() => practice('flashcard')}>🎴 Flashcard</button>
      <button class="px-3.5 py-2 bg-primary text-white border-none rounded-xl cursor-pointer text-sm font-semibold whitespace-nowrap hover:brightness-90 transition-all" on:click={() => practice('multiple-choice')}>✓ Trắc nghiệm</button>
      <button class="px-3.5 py-2 bg-primary text-white border-none rounded-xl cursor-pointer text-sm font-semibold whitespace-nowrap hover:brightness-90 transition-all" on:click={() => practice('typing')}>⌨️ Nhập chữ</button>
    </div>
    <button class="border border-border rounded-lg px-2 py-1 cursor-pointer text-muted-foreground text-sm hover:border-destructive hover:text-destructive transition-colors bg-transparent flex-shrink-0" on:click={clearSelection} title="Bỏ chọn tất cả">✕</button>
  </div>
{/if}
