...
<div class="mx-auto max-w-2xl ">
  ...
  <div class="relative text-white pt-3 pb-6 px-4 overflow-hidden" style="background: var(--color-shell)">
    <div class="relative z-10">
      <h1 class="text-[22px] font-extrabold tracking-tight drop-shadow-sm" style="font-family: var(--font-chinese)">HSK 汉语水平考试</h1>
    ...
    <div class="flex flex-col gap-3">
      {#each groups as group, i}
        ...
        <a
          ...
          class=" group flex items-center gap-4 w-full px-5 py-5 bg-card border border-border/50 rounded-2xl shadow-sm cursor-pointer text-left no-underline transition-colors duration-200 hover:border-primary/50 hover:-translate-y-0.5 hover:shadow-lg hover:bg-accent/30 active:scale-[0.98]"
        ...
        >
          <div class="flex-shrink-0 w-12 h-12 rounded-xl bg-primary/15 flex items-center justify-center group-hover:bg-primary/25 transition-colors">
            <span class="text-lg font-bold text-primary" style="font-family: var(--font-chinese)">{group.id.toUpperCase()}</span>
          </div>
          ...
          <ChevronRight size={20} class="text-muted-foreground transition-transform group-hover:translate-x-1 group-hover:text-primary flex-shrink-0" aria-hidden="true" />
        </a>
      {/each}
    </div>
  </div>
